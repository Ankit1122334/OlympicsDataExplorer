...
def country_year_list(df):
    ...
    country = np.unique(df[
                            "region"].dropna().values).tolist()  # drop nan values because it'sshowing float values so we will do dropna()
    # convert it into list because we can do sort this list
    country.sort()
...
def most_successful(df, sport):
    ...
    x = temp_df['Name'].value_counts().reset_index().head(15).merge(df,how="left")[["Name","count","region","Sport"]].drop_duplicates("Name")
